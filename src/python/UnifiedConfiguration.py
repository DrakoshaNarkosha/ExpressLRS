...
def appendConfiguration(source, target, env):
    ...
    if config is not None:
        ...
        frequency = '2400' if '_2400.' in config else '900' if '_900.' in config else '433' if '_433.' in config else 'dual'
    else:
        moduletype = 'tx' if '_TX_' in target_name else 'rx'
        frequency = '2400' if '_2400_' in target_name else '900' if '_900_' in target_name else '433' if '_433_' in target_name else 'dual'
